...
from autosail.models.native.base import BaseEstimator
...
class LaSVM(BaseKernelMethod, BaseEstimator):
    ...
    def _update_parameters(self, i: int, j: int) -> None:
        ...
        self.gradient = self.gradient - lambda_ * (
            self.kernel_mx[i] - self.kernel_mx[j]
        )
    ...
    def _reprocess(self) -> None:
        ...
        for k in np.where(np.abs(self.alpha) < self.ERR)[0]:
            if (self.target[k] == -1 and self.gradient[k] >= self.gradient[i]) or (
                self.target[k] == 1 and self.gradient[k] <= self.gradient[j]
            ):
                to_remove.append(k)
