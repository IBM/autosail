...
from autosail.common.decorators import log_epoch
from autosail.common.helper import VerboseManager
from autosail.common.progress_bar import SAILProgressBar
from autosail.common.scorer import SAILModelScorer
...
class SAILPipeline(Pipeline):
    ...
    def load(cls, model_folder, name="sail_pipeline"):
        ...
        if Path(os.path.join(estimator_path, ".pytorch")).exists():
            from autosail.models.torch.base import TorchSerializationMixin

            estimator = TorchSerializationMixin.load_model(estimator_path)
        elif Path(os.path.join(estimator_path, ".keras")).exists():
            from autosail.models.keras.base import KerasSerializationMixin
        ...
        if os.path.exists(os.path.join(load_location, "scorer_state.npz")):
            initial_points = np.load(
                os.path.join(load_location, "scorer_state.npz"), allow_pickle=True
            )
